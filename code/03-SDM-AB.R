...
prey <- stack(howler, capuchin, porcupine, slo2, slo3)
prey
plot(prey)

# load abiotic raster data
...
proj4string(prey) <- crs.geo 

env
prey

# add prey models as raster predictors
env <- addLayer(x=env, c(slo2, slo3, howler, capuchin, porcupine))
names(env)
plot(env)
...
# save model as RData
#saveRDS(mn, file = './results/SDM_AB.RData')
...
#plot(ext, add=TRUE, border = "grey30")
...
write.csv(test, "./pROC/testdata_AB.csv")
writeRaster(pred.mn, "./pROC/SDM_AB.asc", overwrite=TRUE)

proc <- PartialROC(PresenceFile="./pROC/testdata_AB.csv", 
                   PredictionFile="./pROC/SDM_AB.asc", 
                   OmissionVal=0.1, 
                   RandomPercent=50, 
                   NoOfIteration=1000,
                   OutputFile="./pROC/TestRoc_SDM_AB.csv") 
